...
class CodeWriter
{
    ...
    public function call($objectVar, $functionName, $args)
    {
        return sprintf(
            '%s->%s(%s)',
            $this->exportFunctionParameter($objectVar),
            $functionName,
            $this->exportFunctionParameter($args)
        );
    }
    ...
    public function exportList(array $list)
    {
      ...
      sprintf(
          'array(%s)',
          A::implode($list, ',', function ($item) use ($that) {
          if (is_array($item)) {
    ...
    public function exportKeyList(array $array, $type = 'keyList')
    {
      ...
      sprintf(
          'array(%s)',
          A::implode($array, ',', function ($item, $key) use ($that, $type) {
          if (is_array($item)) {
